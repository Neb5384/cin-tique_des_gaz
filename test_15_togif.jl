...
N1 = 150000 # nbre de pas pour premiere temperature

T2 = 10.0 # 2 eme temperature
N2 = 150000 # nbre de pas de changement

n_bins = 50   # résolution de la grille pour la distribution de présence
...
#simulation loop ----------------------------------------------------
decouple = max(1, round(Int, n_steps / animation_time / 30))
...
# remplace la boucle interactive (display + while running[]) : on parcourt states_history une seule fois,
# Makie capture une image à chaque itération, puis encode le tout en GIF (le format dépend de l'extension du fichier).
gif_path = "simulation_3phases.gif"

println("enregistrement du gif...")
record(fig, gif_path, 1:length(states_history); framerate = 30) do frame
...
println("gif enregistré : $(gif_path)")